...
/**
 * Set up the mock WebSocket server and ActionCable instance
 * Creates and configures a complete mock environment for testing
 *
 * @returns {{mockServer: Server, actionCableMock: ActionCableMock}} Mock server and ActionCable instances
 * @throws {Error} If server or ActionCable initialization fails
 * @example
 * const { mockServer, actionCableMock } = setupMockActionCable();
 * // Now you can use actionCableMock.subscribe('ChatChannel')
 */
...
/**
 * Teardown and cleanup the mock WebSocket server and ActionCable instance
 * Stops the server, disconnects ActionCable, and removes all global references
 * Call this in afterEach hooks to ensure clean state between tests
 *
 * @returns {void}
 * @example
 * afterEach(() => {
